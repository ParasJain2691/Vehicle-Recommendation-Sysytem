...
double calculateDistance(int id1, int id2) {
    return abs(id1-id2); //It doesn't matter if we square it or not, as We just want minimum 3 distances out of all.
}
...
//Time Complexity : O(n), Parsing the dataset once

//Step-2 : Function that will print the vehicles
// This function will print the 3 vehicles as per the problem statement.
void getRecommendations(int inputLocationId, int numOfPassengers, unordered_map<int, Vehicle>& vehicles) {
    ...
    for (auto& vehiclePair : vehicles) {
        ...
        //If vehichle is not present or It has not required capacity then we should not recommend that vehicle
    ...
    while(!distances.empty()){
        ...
        cout << v1.vehicleNo << ", " << v1.category << ", " << v1.driverContactNo << endl; //printing the results
    }
}

int main() 
{
    //We can use API, CSV files to store the data, But Currently I feel the data isn't that big. So, We can use map for it.
